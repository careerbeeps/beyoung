	...
	PDCExport = {
		...
		renderToCanvas : function() {
            ...
            $.each(sides, function() {
				...
				if(!sideInfo.sideSvg || (sideInfo.sideSvg && sideInfo.sideSvg == 'undefined'))
				{
					var idTabSide = canvasId.replace('canvas_side_','side_');
					$('a[href="#'+idTabSide+'"]').css('display','none');
					return true;
				}
                var _validJson = pdc.checkImagePathInJson(sideInfo.json);
                if(!_validJson) return;
				self.arSideOk[canvasId] = canvasId;
		...
		downloadTxt: function(url) {
            var self = this,
                savePngUrl = url;
            pdc.doRequest(savePngUrl, {}, function(response) {
                pdc.showLog("The png file response to download png event", "info");
                var responseJson = JSON.parse(response);
                if(responseJson.status === "success") {
                    pdc.hideLoadingBar();
                    // window.location = responseJson.thumbnail_path;
					var pdcFileName = responseJson.file_path;
						var arPdfUrl = pdcFileName.split('/');
						var lengthArPdfUrl = arPdfUrl.length;
						lengthArPdfUrl = lengthArPdfUrl - 1;
						pdcFileName = arPdfUrl.slice(-1)[0] ;
        ...
        downloadAll: function() {
            if(allCanvas) {
                ...
                var exportImageUrl = baseUrl + "pdc/download/saveExportImage";
                var allRequest = new Array();
                $.each(allCanvas, function(key, side) {
					if(!self.arSideOk[key] || (self.arSideOk[key] && self.arSideOk[key] == 'undefined'))
					{
						return true;
					}
                    var sidePng = {
                ...
                $.when.apply(null, allRequest)
                    .done(function() {
                        try {
                            ...
                            if(exportFiles) {
                                var downloadAllUrl = baseUrl + "pdc/download/downloadAll";
                                //Final request
            ...
            $('[pdc-data="pdc-export-btn"]').click(function() {
                ...
                switch(_action) {
                    ...
                    case 'DOWNLOAD_JPG' :
                        ...
                        break; 
					case 'DOWNLOAD_TXT' :
						var url = $(this).attr('pdc-data-url');
                        self.downloadTxt(url);
                        break;    
                    case 'DOWNLOAD_ALL' : 
                        self.downloadAll();
                        break;
