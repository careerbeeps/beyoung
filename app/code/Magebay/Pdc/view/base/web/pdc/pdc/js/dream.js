;(function(global, $) {
    ...
    var config = {
        ...
        save_thumbnail_url: $("#base_url").val() + "pdc/save/saveBase64Image/",
        save_json_url: $("#base_url").val() + "pdc/save/savejsonfile/",
        save_admin_template: $("#base_url").val() + "pdc/save/saveAdminTemplate/",
        image_options: {
            format: 'jpeg',
    ...
    PDC.prototype = {
        ...
        saveDesignToCustomerAccount: function() {
            ...
            if(self.checkJSONReady.status == 1) {
                //Ready to save json
                self.exportThumbnaiForMinicart();
        ...
        saveAndContinue: function() {
            ...
            var productId = self.getCurrentProductId();
            if($('input[name="pdc_product_config_json_'+productId+'"]',mainWindow).length)
            {
                $('input[name="pdc_product_config_json_'+productId+'"]',mainWindow).val(self.newestJsonFilename);
            }
        ...
        downloadPng: function() {
            ...
            _canvas.clone(function(cloneCanvas) {
                ...
                self.doRequest(config.base_url + 'pdc/download/png/', {
                    format: 'png',
                    multiplier: 1,
        ...
        downloadPdfFromPng: function() {
            ...
            _canvas.clone(function(cloneCanvas) {
                cloneCanvas.scale = _canvas.scale;
                cloneCanvas = self.pdcZoom.resetZoomBeforeSave(cloneCanvas);
                self.doRequest(config.base_url + 'pdc/download/Pdfpng', {
        ...
        addText: function(text, fontSize) {
            ...
            var textObj = new fabric.IText(text, {
                ...
                price: config.productConfig.note.text_price,
        ...
        saveDesignToJSON: function(customAttrs, currentTarget) {
            if(!Array.isArray(customAttrs)) {
                customAttrs = ['name', 'isrc', 'price', 'object_type', 'selectable', 'scale', 'evented','mk_letter'];
            }
                ...
                $.each(_sidesConfig, function(index, side) {
                    (function(side) {
                        ...
                        if(side.json) {
                            ...
                            var defaultBgColor = side.dm_bgcolor || '#ffffff';
                            _canvas.clear();
                            self.addBackgroundColorLayer(defaultBgColor, {object_type: "background_color"}, _canvas);
                        }
                    })(side);
                });
            }
        ...
        changeDesignTemplate: function(jsonFilename, jsonContent) {
            ...
            {
                $.each(_sidesConfig,function(index,side){
                    var srcBg = side.background_path || '';
                    if(srcBg != '')
                    {
                        self.updateBackgroundInfoToSizeConfig(srcBg,side.id)
                    }
                })
            }
            this.initCanvas();
        ...
        exportThumbnaiForMinicart : function()
        {
            var self = this;
            var mainWindow = top.document;
            if(_sidesConfig) {
                ...
                $.each(_sidesConfig,function(index,side){
                    if(side.sideSvg)
                    {
                        sideId = parseInt(side.id);
                        return false;
                    }
                })
                if(sideId > 0) {
                    var maxWidth = 200;
                    var _canvas = self.allCanvas[sideId];
                    if(_canvas) {
                        _canvas.clone(function(_canvasTemp) {
                            ...
                            self.doRequest( config.save_thumbnail_url, {
                                ...
                                var responseJSON = JSON.parse(response);
                                $('#temp-pdc-thumbnail',mainWindow).val(responseJSON.thumbnail_path);
                            })
                        })
                    }
                }
        ...
        updatePropertyDrawing : function(type, mValue)
        {
            var _canvas = this.getCurrentCanvas();
            if(_canvas.freeDrawingBrush)
            {
                if(type == 'draw_type')
                {
                    var drawColor = $('#drawing-color-value').val();
                    var lineWidth = $('#drawing-line-width').val();
                    if (mValue === 'hline') {
                        _canvas.freeDrawingBrush = this.vLinePatternBrush;
                    }
                    else if (mValue === 'vline') {
                        _canvas.freeDrawingBrush = this.hLinePatternBrush;
                    }
                    else if (mValue === 'square') {
                        _canvas.freeDrawingBrush = this.squarePatternBrush;
                    }
                    else if (mValue === 'diamond') {
                        _canvas.freeDrawingBrush = this.diamondPatternBrush;
                    }
                    else if (mValue === 'texture') {
                        _canvas.freeDrawingBrush = this.texturePatternBrush;
                    }
                    else
                    {
                        _canvas.freeDrawingBrush = new fabric[mValue + 'Brush'](_canvas);
                    }
                    _canvas.freeDrawingBrush.color = drawColor;
                    _canvas.freeDrawingBrush.width = parseInt(lineWidth, 10) || 1;
                    //_canvas.freeDrawingBrush.shadowColor = '#C11919';
                    //_canvas.freeDrawingBrush.shadowBlur = 50;
                }
                else if(type == 'color')
                {
                    _canvas.freeDrawingBrush.color = mValue;
                }
                else if(type == 'line_width')
                {
                    _canvas.freeDrawingBrush.width = parseInt(mValue, 10) || 1;
                }
            }
        },
        intallizeDrawingPdc : function()
        {
            $('#drawing-attribute-content').css('display','block');
        ...
        stopDrawingPdc : function()
        {
            var canvas = this.getCurrentCanvas();
            $('#drawing-attribute-content').css('display','none');
    ...
    PDC.init = function() {
                                ...
                                self.saveJsonFile(_saveSampleOptions, function(response) {
                                    ...
                                    if(responseJSON.status == "error") {
                                        ...
                                        try {
                                            ...
                                            //http://localhost:8888/m2sample/pdc/designarea/index/productid/1/#/templatelist/product/1
                                            var	url = $("#base_url").val() + "pdc/designarea/index/productid/" + self.getCurrentProductId() +"/#/templatelist/product/" + self.getCurrentProductId();
                                            window.location = url;
                                        } catch (error) {
                                            console.error(error);
                                        }
        ...
        $('[pdc-data="SAVE_CUSTOMER_DESIGN"]').click(function() {
            var exitOjbect = false;
            $.each(self.allCanvas, function(mKey,_canvas) {
                if(self.hasDesignItem(_canvas))
                {
                    exitOjbect = true;
                    return true;
                }
            ...
            if(!exitOjbect)
            {
                alert('There is no design item. Please add text or image to design!');
                return false;
            }
        ...
        $('#start-drawing').click(function(){
            ...
            self.intallizeDrawingPdc();
        })
        $('#stop-drawing').click(function(){
            self.stopDrawingPdc();
        })
        $('.pdc-color-list-frawing a').click(function(){
            var valueColor = $(this).attr('pdc-data-color');
            $('#drawing-color-value').val(valueColor);
            self.updatePropertyDrawing('color',valueColor)
        })
        $('#drawing-line-width').change(function(){
            var lineWidth = $(this).val();
            $(this).prev().html(lineWidth);
            self.updatePropertyDrawing('line_width',lineWidth);
        })
        $('#drawing-mode-selector').change(function(){
            var modelvalue = $(this).val();
            self.updatePropertyDrawing('draw_type',modelvalue);
        })
                ...
                $.each(_sidesConfig, function(index, _side) {
                    (function(side) {
                        ...
                        if(side.use_mask == '1')
                        {
                            ...
                            var mask = config.media_url + side.overlay;
