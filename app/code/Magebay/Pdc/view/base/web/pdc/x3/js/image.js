...
x3Image(function($) {
        ...
        uploadImageDropzone.on("success", function(file, response) {
            try {
                ...
                if(responseJson.status == "success") {
                    ...
                    var imgItem = '<li class="'+ imgClass +'"><span class="price">'+ (responseJson.price_format || "Free") +'</span> <div class="del-upload-image-btn"><span title="Delete">x</span></div><a class="uploaded-img"><img object-type="upload_image" rel="'+ responseJson.filename +'" src="'+ (responseJson.thumbnail || responseJson.filename) +'" price="'+ responseJson.price +'"></a><a href="#" class="upload-image-crop" src-image-crop="'+responseJson.filename+'">Crop</a></li>';
    ...
    PDCImage = {
        ...
        removeUploadImage: function() {
            $(document).on("click", '.del-upload-image-btn', function() {
                if(confirm("Are you sure?")) {
					...
					if($('[pdc-image-type="upload_image"] img[rel="'+ removeImg +'"]').length) {
						$('[pdc-image-type="upload_image"] img[rel="'+ removeImg +'"]').closest("li").remove();
					}
                    
                }
            }); 
        }(),
		intPdcCropImage : function()
		{
			var self = this;
			$(document).on('click', '.upload-image-crop', function() {
				var srcImage = $(this).attr('src-image-crop');
				$("#crop_image").attr("src", srcImage);
				$("#crop_image").Jcrop({
						onSelect: function(c) {
			...
			$('#image-crop-submit').click(function(){
				$(this).text('croping...')
				self.sendCropRequest(window.pdcCropSelection);
			})
			$('#image-crop-cancel').click(function(){
				self.destroyJcrop();
			})
		},
		sendCropRequest : function(data) {
            ...
            var cropUrl = baseUrl + "pdc/upload/cropImage";
            $.ajax({
				type : "POST",
				url : cropUrl,
				data : data,
				...
				success : function(response) {
					if(response != "") {
                        var responseJson = JSON.parse(response);
                        if(responseJson.status === "success") {
                            var imgItem = '<li class="valid"><span class="price">'+ (responseJson.price_format || "Free") +'</span> <div class="del-upload-image-btn"><span title="Delete">x</span></div><a class="uploaded-img"><img object-type="upload_image" rel="'+ responseJson.crop_image +'" src="'+ (responseJson.crop_image) +'" price="'+ responseJson.price +'"></a><a href="#" class="upload-image-crop" src-image-crop="'+responseJson.crop_image+'">Crop</a></li>';
							$("#upload ul.items-list").prepend(imgItem);
                           $('#image-crop-submit').text("Crop");
						   self.destroyJcrop();
						   $.fancybox.close();
                        } else if(responseJson.status === "error") {
                            // alert(responseJson.message);
							$('#pdc-crop-image-msg').html('You can not crop image');
                        }
						pdc.hideLoadingBar();
                    }
				}
		...
		destroyJcrop : function() {
            $("#pdc-crop-image #crop_image").data('Jcrop').destroy();
			$("#pdc-crop-image #crop_image").attr("src",'');
			pdc.hideLoadingBar();
			$.fancybox.close();
        },
	}
