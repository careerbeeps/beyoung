...
pdc_layer(document).ready(function($){
    ...
    PDC_layer = {
        init: function(){
			...
			$(document).on('click', '#pdc-layer-forward-img', function() {
				self.layerForward();
			})
			$(document).on('click', '#pdc-layer-backward-img', function() {
				self.layerBackWard();
			})
        },
        ...
        load_layer: function(){ 
            ...
            if(hasDesignItem) {
				...
				{
					for(var j = arrayObjects.length - 1; j >= 0; j--)
					{
						var tempObj = arrayObjects[j];
						if(tempObj.object_type != 'background_color')
						{
								if((tempObj.price=='')||(tempObj.price==undefined)){tempObj.price=0;}
								 var o_src = tempObj.isrc || tempObj.src;
								 if(o_src && o_src != 'undefined')
								 {
									  var arO_src = o_src.split('pub/media');
									 if(arO_src[0] != baseUrl)
									 {
										o_src = baseUrl +'pub/media'+arO_src[1];
									 }
								 }
								price+=parseFloat(tempObj.price);
								$('[pdc-block="layer"] tbody').append('<tr obj_type="'+tempObj.object_type+'" pdc-layer="'+j+'" name="'+tempObj.name+'">'+html+'</tr>');
								if((tempObj.type=='text')||(tempObj.type=='i-text')||(tempObj.type=='curvedText')){
									$('[pdc-layer="'+j+'"] [pdc-layer-info="type"]').html(tempObj.text.substring(0,10));
								} 
								if((tempObj.type=='image')||(tempObj.type=='path-group')){                             
									$('[pdc-layer="'+j+'"] [pdc-layer-info="type"]').html('<img src="'+o_src+'" />');
								}
								if(tempObj.object_type=='background'){
									$('[pdc-layer="'+j+'"] [pdc-layer-info="lock"] i').hide();
									//$('[pdc-layer="'+i+'"] [pdc-layer-info="type"]').html('<span class="pdc_layer_bg" style="background-color:'+o.fill+'"></span>');
								}
								$('[pdc-layer="'+j+'"] [pdc-layer-info="price"]').html(currency_symbol+parseFloat(tempObj.price).toFixed(2));
								$('[pdc-layer="'+j+'"] [pdc-layer-info="size"]').html(parseInt(tempObj.width*tempObj.scaleX) + 'X' + parseInt(tempObj.height*tempObj.scaleY));
						}
						else
						{
							 $('#pdc_block_layer').attr('bg_color',tempObj.fill);
						}
					}
				}
		...
		layerBackWard : function()
		{
			$('.a-layers').click();
			var _canvas = pdc.getCurrentCanvas();
			var active = _canvas.getActiveObject();
			var objIndex = _canvas.getObjects().indexOf(active),
			minIndex = 0;
